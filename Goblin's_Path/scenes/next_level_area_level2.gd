...
func _on_body_entered(body):
	...
	if body.name == "Goblin":
		...
		get_tree().call_deferred("change_scene_to_file", "res://Goblin's_Path/scenes/GameComplete.tscn")
		print_debug("DEBUG: Level 2 Completed")
...
func fade_in(time = 0.5) -> void:
	...
	box.color = Color(0, 0, 0, 1) # idk y but u need or it'll break
	box.modulate.a = 0.0
	var tween = create_tween()
